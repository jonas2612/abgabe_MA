...
if sys.argv[1]=='atlas':
    adata = sc.read_h5ad(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/training/1M/{sys.argv[2]}/2/2048/{sys.argv[3]}_adata.h5ad')
    cluster_key = 'cellcluster_moscot'
    n_states = int(len(adata.obs.cellcluster_moscot.unique())*1.0)
else:
...
if sys.argv[3] == 'unintegrated' and sys.argv[1]=='atlas':
    x_key = 'X_pca'
else:
    x_key = 'X_emb'
...
with open(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/reference_query/divergences/flow/1M/{sys.argv[2]}/2/2048/{sys.argv[3]}_cond_model{end}.pt', mode='rb') as file:
...
pd.DataFrame({'terminal_states_found': [terminal_states_found], 'correct_terminal_states': [corrcet_terminal_states]}).to_csv(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/downstream_analysis/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}/{sys.argv[4]}/terminal_cells.csv')

g.set_terminal_states(terminal_state_anno)

g.compute_fate_probabilities(tol=1e-20)
try:
    g.plot_fate_probabilities(same_plot=False, save=f'/home/icb/jonas.flor/gastrulation_atlas/scvi/downstream_analysis/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}/{sys.argv[4]}/fate_probabilities.png')
except ValueError:
    g.plot_fate_probabilities(same_plot=True, save=f'/home/icb/jonas.flor/gastrulation_atlas/scvi/downstream_analysis/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}/{sys.argv[4]}/fate_probabilities.png')
...
cr.pl.aggregate_fate_probabilities(
    ...
    title="",
    save=f'/home/icb/jonas.flor/gastrulation_atlas/scvi/downstream_analysis/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}/{sys.argv[4]}/absorption_probability.png'
)
...
for t in g.terminal_states.cat.categories.astype('string'):
    try:
    	...
    	driver_genes.to_csv(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/downstream_analysis/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}/{sys.argv[4]}/{t.replace("/","_")}_lineage_driver.csv')
    except RuntimeError:
        pd.DataFrame(data={'Terminal_States':g.terminal_states.cat.categories.astype('string')}).to_csv(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/downstream_analysis/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}/{sys.argv[4]}/no_lineage_drivers.csv')
