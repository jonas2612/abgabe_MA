...
import pandas as pd
import sys
#TODO check all imports and writes when model selected
...
sc.tl.umap(adata)
sc.pl.umap(adata, color='day', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_pca_1025_day.png')
sc.pl.umap(adata, color='cellcluster_moscot', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_pca_1025_cell.png')
...
if sys.argv[3]=='integrated':
    ...
    adata_loader = OTDataLoader(1024, source_lin=adata[adata.obs['day']==10.25].obsm['X_pca'], target_lin=adata[adata.obs['day']==10.5].obsm['X_pca'])
else:
    raise NotImplementedError
...
with open(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/reference_query/divergences/flow/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}_1025_model.pt', mode='wb') as file:
   cloudpickle.dump(fm, file)
#fm.save(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/reference_query/divergences/flow/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}_1025_model.pt')
...
if sys.argv[3]=='integrated':
    adata_concat.obsm["X_new"] = np.concatenate(
        (adata[adata.obs['day']==10.25].obsm['X_emb'],
         np.array(fm.transport(jnp.array(adata[adata.obs['day']==10.25].obsm['X_emb']), condition=None, forward=True)), 
    ...
    adata_concat.obsm["X_new"] = np.concatenate(
        (adata[adata.obs['day']==10.25].obsm['X_pca'],
         np.array(fm.transport(jnp.array(adata[adata.obs['day']==10.25].obsm['X_pca']), condition=None, forward=True)), 
...
adata_concat.write(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/reference_query/divergences/flow/{sys.argv[1]}/{sys.argv[2]}/{sys.argv[3]}_1025_adata.h5ad')
sc.pl.umap(adata_concat, color='day_umap', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_all_1025_day.png')
sc.pl.umap(adata_concat, color='cellcluster_moscot', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_all_1025_cell.png')
...
sc.pl.umap(adata_con_2, color='day_umap', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_pushed_1025_day.png')
sc.pl.umap(adata_con_2, color='cellcluster_moscot', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_pushed_1025_cell.png')

divergence = pd.DataFrame(
    data={'n_obs': [sys.argv[1]], 'n_genes': [sys.argv[2]], 'time': ['10.25, 10.50'],
...
).to_csv(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/reference_query/divergence_results/flow/divergence_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_1025.csv')
