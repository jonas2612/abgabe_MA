...
adata = adata[[x in [18.5, 12] for x in adata.obs.day]]
...
sc.tl.umap(adata)
sc.pl.umap(adata, color='day', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_pca_12.png')
...
if sys.argv[3]=='integrated':
    adata_concat = sc.concat(
        [adata[adata.obs.day==12], adata[adata.obs.day==18.5]],
    )  
    adata_concat.obsm["X_new"] = np.concatenate(
        (np.array(problem[list(problem)[0]].solution.push(adata[adata.obs.day==12].obsm["X_emb"])), adata[adata.obs.day==18.5].obsm['X_emb']),
        axis=0
    )
elif sys.argv[3]=='unintegrated':
    adata_concat = sc.concat(
    ...
    adata_concat.obsm["X_new"] = np.concatenate(
        (np.array(problem[list(problem)[0]].solution.push(adata[adata.obs.day==12].obsm["X_pca"])), adata[adata.obs.day==18.5].obsm['X_pca']),
        axis=0
    )
...
sc.pl.umap(adata_concat, color='day', save=f'_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_pushed_12.png')

divergence = pd.DataFrame(
    data={'n_obs': [sys.argv[1]], 'n_genes': [sys.argv[2]], 'time': ['12.0, 18.5'],
...
).to_csv(f'/home/icb/jonas.flor/gastrulation_atlas/scvi/reference_query/divergence_results/moscot_not/divergence_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_12.csv')