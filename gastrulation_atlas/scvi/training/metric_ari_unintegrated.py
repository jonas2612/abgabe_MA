...
scib_metrics = pd.DataFrame(
    data={'n_obs': [adata.n_obs], 'n_cells': [sys.argv[1]], 'n_genes': [sys.argv[2]], 'n_layers': [sys.argv[3]], 'n_hidden': [sys.argv[4]], 'metric': ['ari_unintegrated']}
)
...
scib_metrics.to_csv(f'{adata_path}/metrics/metrics_ari_unintegrated.csv')
